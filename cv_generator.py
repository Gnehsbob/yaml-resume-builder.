...
# ============ HELPER TO BUILD SOCIAL LINKS ============
def format_social_link(handle_or_url, base_url):
    """If handle_or_url looks like a full URL, use it as is; otherwise prepend base_url."""
    if handle_or_url.startswith(('http://', 'https://')):
        return handle_or_url
    # If it already contains a domain (e.g., "github.com/username"), just add https://
    if '.' in handle_or_url.split('/')[0]:  # crude check for domain presence
        return f"https://{handle_or_url}"
    # Otherwise assume it's just the username and build the full URL
    return f"{base_url}/{handle_or_url}"
...
def build_cv(output_filename='your_cv.pdf'):
    ...
    # Build GitHub and LinkedIn links dynamically from the YAML data
    github_link = format_social_link(contact.get('github', ''), 'https://github.com')
    linkedin_link = format_social_link(contact.get('linkedin', ''), 'https://linkedin.com/in')

    contact_text = (
        f"{contact['phone']} | {contact['email']} | {contact['location']}<br/>"
        f'<a href="{github_link}">GitHub</a> | '
